jQuery(function($) {
    ...
    $.ajax({
      url: "https://server.usbekistanreise.uz/api/v1/carousels", 
      method: "GET",
      success: function (response) {
        loadCarouselImages(response.data); 
      },
    });

    function loadCarouselImages(data) {
      var carousel = $(".banner-slider-three");
      carousel.trigger("destroy.owl.carousel").html("");
      data.forEach(function (imageUrl) {
        carousel.append(
          '<div class="slider-item"><img src="' +
            imageUrl.image_url +
            '" width="100%"/></div>'
        );
      });
...
console.log(data)
      data.forEach(function (testimonial) {
        testimonialSlider.append(
          '<div class="slider-item">' +
            ...
            "</p>" +
            '<div class="review">' +
            // Adjust the number of stars based on a rating property in the data
            '<i class="bx bxs-star"></i>'.repeat(testimonial.rating) +
            '<i class="bx bxs-star"></i>'.repeat(5 - testimonial.rating) +
            "</div>" +
            "</div>" +
            "</div>"
        );
      ...
      // Reinitialize the testimonial slider after adding new items
